...
description: "Learn the most important safety distinction online: which information is private (and must be protected) and which is personal (and is safe to share)."
...
<details markdown="1">
<summary>Sign-Up Form Sorter — interactive p5.js MicroSim</summary>
Type: microsim
**sim-id:** signup-form-sorter<br/>
**Library:** p5.js<br/>
**Status:** Specified
...
- A pretend sign-up form on the left side of the canvas, drawn as a clean kid-friendly card with rows for fields like *Favorite color*, *Full name*, *Hobby*, *Home address*, *Birthday*, *Country*, *Phone number*, and *School name*.
...
- A small padlock area at the bottom that shows a closed padlock icon next to a fake `https://` URL, with a one-line explanation of what the padlock means.

**Controls (built-in p5.js controls per project rules, placed at the bottom of the canvas):**

- `createButton('New form')` to load a new pretend form with different fields shuffled in.
- `createButton('Reset')` to clear the score.
- `createSelect()` to switch the example URL between an `https://` (with padlock) version and an `http://` (no padlock) version, so students can see the difference.
...
- File location: `docs/sims/signup-form-sorter/` with `main.html`, `main.js`, and `index.md`.
...
1. **HTTPS** — the safer version of a website address that starts with `https://`
2. **Personal information** — friendly facts about you, usually safe to share
3. **Favorite color** — a tiny piece of personal information
4. **Hobby information** — what you like to do, usually safe to share
5. **Padlock icon** — the small lock shown in the address bar for HTTPS sites
6. **Private information** — facts that could let strangers find or fake you
7. **Birthday** — your exact date of birth, private
8. **Full name** — your first, middle, and last name together, private
9. **Home address** — the street and number where you live, private
10. **Location sharing** — when an app tells someone where you are
11. **Phone number** — the digits to reach your family's phone, private
12. **School name** — the name of your school, private
13. **Sign up form** — the page where you make a new account
14. **GPS** — how a device knows exactly where it is on the planet
15. **Identifying information** — any mix of facts that points to the real you
